...
def shared_protection(topology_file, traffic_file):
    ...
    def draw_network(G, pos, edge_flows, title):
        ...
        plt.title(title)
        plt.savefig('objective_data_fig/optimized_network_topology-shared-'+traffic_file_name+'.png')
        plt.show()
    ...
    def allocate_spectrum_shared(request, path, flow, spectrum_usage,spectrum_usage_shared):
        fsu_needed = int(math.ceil(flow / CABLE_CAPACITY)) * FSU_PER_CABLE
        fsu_availability = [float('inf')] * (FSU_CAPACITY - fsu_needed + 1)
        ...
        for i in range(len(path) - 1):
            edge = (path[i], path[i + 1])
            for j in range(fsu_index, fsu_index + fsu_needed):
                spectrum_usage_shared[edge][j] += 1

        return fsu_index, fsu_needed, spectrum_usage_shared
    ...
    spectrum_usage_shared = copy.deepcopy(spectrum_usage)
    for request_backup, path_backup in best_state_backup.items():
        source, target = request
        flow = traffic_matrix[source - 1][target - 1]
        fsu_index_backup, fsu_needed_backup, spectrum_usage_shared = allocate_spectrum_shared(request_backup, path_backup, flow, spectrum_usage,spectrum_usage_shared)
    ...
    def draw_spectrum_allocation(best_state, best_state_backup, spectrum_allocation, spectrum_allocation_backup):
        ...
        plt.savefig('objective_data_fig/spectrum_allocation-shared-' + traffic_file_name + '.png')
        plt.show()

    draw_spectrum_allocation(best_state, best_state_backup, spectrum_allocation, spectrum_allocation_backup)

    with open('objective_data_fig/spectrum_allocation-shared-'+traffic_file_name+'.pkl', 'wb') as f:
        pickle.dump(spectrum_allocation, f)
        pickle.dump(edge_traffic_details, f)
        pickle.dump(spectrum_usage, f)


    def performance_calculation(spectrum_usage_shared,best_state,G,COST_PER_CABLE,FSU_PER_CABLE):
        num_fsu_used = 0
        highest_load=0
        highest_fsu=0
        total_length=0
        total_cost=0
        for request in spectrum_usage_shared:
            num_fsu_used+=sum(spectrum_usage_shared[request])
            if sum(spectrum_usage_shared[request])>highest_load:
                highest_load=sum(spectrum_usage_shared[request])
            try:
                if (len(spectrum_usage_shared[request]) - spectrum_usage_shared[request][::-1].index(1))>highest_fsu:
                    highest_fsu=(len(spectrum_usage_shared[request]) - spectrum_usage_shared[request][::-1].index(1))
            except:
                pass
        average_fsu_used=math.ceil(num_fsu_used/len(spectrum_usage_shared))
    ...
    for request in spectrum_usage_shared:
        for i in range(len(spectrum_usage_shared[request])-1):
            if spectrum_usage_shared[request][i]!=0:
                spectrum_usage_shared[request][i]=1
    print(performance_calculation(spectrum_usage_shared,best_state,G,COST_PER_CABLE,FSU_PER_CABLE))
    return performance_calculation(spectrum_usage_shared,best_state,G,COST_PER_CABLE,FSU_PER_CABLE)

if __name__ == '__main__':
    num_fsu_used,highest_load,highest_fsu,average_fsu_used,average_path_length,total_cost=shared_protection('G7-topology.txt', 'G7-matrix-5.txt')
    # print("Done")
